...
function Search() {
  ...
  useEffect(() => {
    if (searchState.query.trim() != "") {
      axios
        .post("/api/search", { name: searchState.query })
        .then((res) => {
          let tempArr = [];
  ...
  function clickSearch(name) {
    ...
    axios.get(`/api/network/${name}`).then((res) => {
      let promNetwork = new Promise((resolve, reject) =>
        resolve(setCurNetwork(res.data))
      );
      promNetwork.then(() => {
        setIsShowing(true);
      });
  ...
  return (
    <div className="search">
      <div className="search__bar">
        <label htmlFor="search__input" style={{ textIndent: "-9999px" }}>
          Search for Network
        </label>
        <input
          ref={inputEl}
        ...
        <button
          ...
          onClick={() => clickSearch(resultState[0])}
        >
          Search
        </button>
      </div>
